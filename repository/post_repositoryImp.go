...
func (repository *postRepositoryImp) Save(ctx context.Context, post domain.Post) (domain.Post, error) {
	...
	if err != nil {
		return post, fmt.Errorf("repo :%w", err)
	}
	id, err := result.LastInsertId()
	if err != nil {
		return post, fmt.Errorf("get lastInsertId: %w", err)
	}
...
func (repository *postRepositoryImp) Update(ctx context.Context, post domain.Post) (domain.Post, error) {
	tx, err := repository.DB.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollBack(tx)
